...
- 회원정보, 영화정보, 주문정보 와 같은 복합 데이터는 낱개의 변수를 사용하여 다루는 것보다 별도의 타입을 정의하여 "한 단위로 묶어서" 다루는것이 편하다.(규모가 커지면 사용한다, 왜냐 코드를 좀더 체계적으로 관리하고 싶기 때문에)
  - 복합데이터: 여러개의 항목으로 구성된 데이터
  예) 회원정보(번호, 이름, 전화, 주소 등) , 영화정보(영화명, 설명, 제작사, 주연배우, 개봉일 등)

...
public class WebApplicationInitializer implements ServletContextListener {
  ...
  public void contextInitialized(ServletContextEvent sce) {
    // 1) DispatcherServlet이 사용할 의존 객체(페이지 컨트롤러, 서비스, DAO 등)을 
    // 관리하는(생성 - 소멸)하는 bean container(Ioc Container)를 준비한다.
    ...
    registration.setLoadOnStartup(1); // 웹 애플리케이션이 시작 될 때 프론트 컨트롤러를 자동 생성하라는 의미
    registration.addMapping("/app/*"); // 클라이언트 에서 /app/* 요청이 들어 왔을때 프론트 컨트롤러에게 전달하라는 의미 
  }
}

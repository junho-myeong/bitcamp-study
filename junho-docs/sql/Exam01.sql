...
DB 객체(테이블, 뷰, 함수, 트리거 등)을 생성, 변경, 삭제하는 SQL명령이다.
- > create...;
- > alter....;
- > drop....;
...
ex) > create database testdb character set utf8 collate utf8_general_ci;
...
ex) > create table test01 (
  name varchar(50) not null, /* null 값을 허용하지 않는다 */
  kor int not null,
  eng int not null,
  math int not null,
  sum int not null,
...
## 테이블 정보 보기
> desc table 테이블명;
ex) desc table test01;

## 테이블 삭제하기
> drop table 테이블명;
ex) drop table test01;
...
데이터를 입력하지 않아도 된다.
create table test1(
  no int, /* null 옵션 부분에 아무것도 없을때는 기본적으로 null을 허용한다는것이다*/
  name varchar(20)
);

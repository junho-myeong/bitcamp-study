...
public class Exam0130 {
  ...
  static class A {
    ...
    // - static이 붙은 변수이기 떄문에 "스태틱 변수"라고도 부른다.
  ...
  public static void main(String[] args) {
    ...
    // 스태틱 변수와 인스턴스 변수는 메모리가 준비되면 바로
    // 초기값을 넣어 준다. int경우 0, String에 경우 null, float경우 0.0 이 넣어진다. 
    A.v1 = 100;
    A.v2 = true;
  ...
  //클래스 변수는 클래스가 로딩될 때 생성되기 때문에 
  //클래스에 대해 딱 한 번 자동으로 생성된다.
  //따라서 인스턴스 변수 처럼 같은 클래스에 대해 여러개의 개별 데이터를 저장할수 없다.
  //왜? 변수가 한 개이다.
  ...
  //    예) Integer.parseInt(..); // 스태틱 변수 paresInt()사용
  ...
  //   - JVM을 실행할 때 지정한 CLASSPATH 환경 변수에 설정된 디렉토리를 탐색하여 찾는다.
  ...
  //   - 로딩된 클래스가 참조하는 외부 클래스나 인터페이스가 유효성을 검사한다.
  //6) 클래스 초기화시키기
  ...
  //     - 바이트코드를 그대로 메모리에 두는것이 아니라, 맴버에 종류에 따라 적절하게 분류한다.

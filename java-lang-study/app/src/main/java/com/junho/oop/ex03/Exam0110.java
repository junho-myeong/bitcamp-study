...
public class Exam0110 {
  ...
  // 이 예제의 목표는 인스턴스 변수이다.
  static class A { // 클래스 사용하는 이유1: 사용자 정의 데이터 타입
    // 인스턴스 변수
    // => new 명령을 통해 생성된다.
  ...
  public static void main(String[] args) {
    ...
    // A클래스에 대해 new명령을 사용하기 전에는 v1,v2 메모리는 존재하지 않는다.
    // 단지 설계도일 뿐이다.
    A obj1 = new A(); // A클래스 에서 변수 선언 명령을 실행한다. 주의 !! 메서드 정의는 실행하지 않는다.
    A obj2 = new A();
    A obj3 = new A();

    // 이렇게 생성된 메모리를 "인스턴스", "객체"라고 부른다.
    // 이 인스턴스의 주소를 저장하는 obj1, obj2, obj3을 "레퍼런스"라 부른다.
    // 인스턴스가 생성될 때 만들어지는 v1,v2를 "인스턴스 변수"라 부른다.

    // 인스턴스 변수는 레퍼런스를 통해 접근할 수 잇다.
    obj1.v1 = 100;
    obj2.v1 = 200;
    obj3.v1 = 300;
  ...
  // 인스턴스 변수는 new 명령을 실핼할 때 마다 생성되기 때문에

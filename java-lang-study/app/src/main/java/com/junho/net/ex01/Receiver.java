...
public class Receiver {
  public static void main(String[] args) throws Exception {
    ...
    // - new ServerSocket(포트번호) -> 대기열을 생성하는 코드라고 보면된다.
    // - 포트번호?
    //      - 호스트에서 실행중인 서버 프로그램을 구분하는 번호
    //      - 1024 ~ 49151 사이의 값 사용한다.
    //          - 1 ~ 1023 사이의 포트번호는 특정 서버가 사용하기 위해 미리 예약 된 번호다.
    //          - 가능한 이 범위의 포트 번호는 사용하지 않는것이 좋다.
    //      - 유명 프로그램에 포트 번호도 가능한 사용하지 말라!
    //          - 예) Oracle DBMS(1521), MySQL DBMS(3306)등 
    //      - 같은 컴퓨터에서 다른 프로그램이 이미 사용중인 포트 번호는 지정할 수 없다.
    //          - 포트 번호는 중복으로 사용 될수 없다.
    ServerSocket serverSocket = new ServerSocket(8888); // 클라이언트 접속 대기중(대기열을 만드는것)
    System.out.println("ServerSocket 생성!");

    // 2) 연결을 기다리고 잇는 서버가 있다면 맨 먼저 접속한 클라언트의 연결을 승인한다.
    //  - 클라이언트가 서버에 접속을 요청하면 그 정보를 "대기열"이라고 불리는 목록으로 관리한다.
    //  - accept()를 호출하면 대기열에서 순서대로 꺼내 해당 클라이언트와 연결된 소켓을 만든다.
    Socket socket = serverSocket.accept(); // 대기열에 클라이언트 접속 요청이 없으면 여기서 계속 기다린다.
    System.out.println("클라이언트와 연결된 Socket 생성!");

    // 3) 소켓 객체를 읽고 쓸 수 있도록 입출력 스트림을 얻는다.
    //  - 연결된 클라이언트로 데이터를 보내고 받으려면 입출력 스트림을 꺼내야한다.
    //  - 소켓이 리턴해준 입출력 스트림에 적절한 데코레이터를 붙여서 사용한다.
    PrintStream out = new PrintStream(socket.getOutputStream());
    Scanner in = new Scanner(socket.getInputStream());
    System.out.println("데이터 송수신을 위한 입출력 스트림 준비!");

    // 4) 상대편이 보낸 문자열을 한줄읽는다.
    ...
    out.println("보내는 서버는 명준호 서버");

    // 6) 항상 입출력 도구는 사용 후 닫아야한다.
    in.close();
    out.close();

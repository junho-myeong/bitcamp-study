...
public class ClientApp {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      int slashPosition = input.indexOf("/"); // 예) +/100/200, -/100/200, //200/100
      String serverAddress = input.substring(0, slashPosition);
      String queryString = input.substring(slashPosition + 1);
      ...
      if (queryString.startsWith("/")) {
        queryString = queryString.replaceFirst("/", "%2f"); // 서버에 보내는 데이터 형식에 어긋나지 않도록 encoding한다
      }
      // 서버에 데이터를 보낸다.

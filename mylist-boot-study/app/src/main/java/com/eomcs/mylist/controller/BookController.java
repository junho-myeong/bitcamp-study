...
public class BookController {
  ...
  public ResponseEntity<Resource> photo(String filename) {
    try {
      // 다운로드할 파일에 입력 스트림 자원을 준비한다.
      File downloadFile = new File("./upload/book/" + filename); // 다운로드 파일에 상대 경로 준비
      FileInputStream fileIn = new FileInputStream(downloadFile.getCanonicalPath()); // 다운로드 파일에 실제 경로를 지정하여 입력 스트림 준비
      InputStreamResource resource = new InputStreamResource(fileIn); // 입력 스트림을 입력 자원으로 포장

      // HTTP 응답 헤더를 준비한다.
      HttpHeaders header = new HttpHeaders();

      // 만약 다운로드 파일을 지정하고 싶다면 다음의 응답헤더를 추가할
      ...
      return ResponseEntity.ok() // HTTP 응답 프로토콜에 따라 응답을 수행할 생성기를 준비한다.
          ...
          .contentLength(downloadFile.length()) // 응답할 파일에 크기를 설정한다.
          .contentType(MediaType.APPLICATION_OCTET_STREAM) // 응답 컨텐트에 MIME 타입을 설정한다.
          .body(resource); // 응답 컨텐트를 생성한후 리턴한다.
    } catch (Exception e) {
      e.printStackTrace();
      return null;
    }

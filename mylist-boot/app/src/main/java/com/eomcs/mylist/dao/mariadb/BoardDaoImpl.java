...
public class BoardDaoImpl implements BoardDao {
  ...
  @Autowired // 스프링 부트가 보관하고 있는 객체 중에서 다음 타입에 객체가 있다면 자동으로 주입해줄것을 지시하는 애노테이션
  ...
  public int countAll() {
    try (Connection con = dataSource.getConnection();
        // DataSource에서 얻은 커넥션 객체는 close() 할 때 연결을 끊는것이 아니라 DataSource에 반납된다.
        PreparedStatement stmt = con.prepareStatement( //
            "select * from ml_board");

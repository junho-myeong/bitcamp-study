...
public class Receiver {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 포트번호: 
    //   - 호스트(애플리케이션을 실행하는 컴퓨터)에서 실행 중인 서버 프로그램을 구분하는 번호
    ...
    // 2) 연결을 기다리고 잇는 클라이언트가 있다면 맨 먼저 접속한 클라이언트의 연결을 승인한다.
    // - 클라이언트가 서버에 접속을 요청하면 그 정보를 "대기열"이라고 불리는 목록으로 관리한다.
    // - accept()를 호출하면 대기열에서 순서대로 꺼내 해당 클라이언트와 연결된 소켓을 만든다.
    // - 대기열에 클라이언트 연결이 들어오면 accept()메서드를 통해 socket객체를 리턴한다.

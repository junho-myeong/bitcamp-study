...
public class Exam0210 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      // JVM에서 jdbc driver 파일(.jar/META-INF/services/java.sql.Driver )을 뒤져서 
      // java.sql.Driver 를 구현한 클래스를 알아내어 자동으로 로딩하기 떄문에
      // 다음과 같이 명시적으로 로딩할 필요가 없다.
      // Class.forName("org.mariadb.jdbc.Driver");

      // DBMS에 연결하기
      // => Driver 객체를 직접 사용하여 DBMS에 연결하지 않고 대신DriverManager를 통해 연결한다.
      // => DriverManager에게 연결할 DBMS의 정보를(jdbc url)을 주면
      //        해당 DBMS와 관련된 Dirver 객체를 찾아 connect()를 호출한다.
      // jdbc url : DBMS 서버 정보. 

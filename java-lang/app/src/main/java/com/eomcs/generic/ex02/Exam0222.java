...
public class Exam0222 {
  ...
  static void m1(ArrayList<?> list) {
    // 컴파일러가 문법을 검사 할 때는 실제 파라미터에 어떤 값이 넘어 왔는지 따지지 않는다.
    // 왜? 실행 상태가 아니기 때문에 레퍼런스에 전달된 객체가 무엇인지 알수 없기 때문이다.

    // 그래서 컴파일러는 문법 검사를 할 때 글자 그대로 검사한다.
    // 즉 레퍼런스에 지정된 제네릭 타입으로만 유효 여부를 검사한다.
    //
    // 위의 list 파라미터는 제네릭 타입이 ?로 설정되어 있다.
    // 즉 ArrayList에 타입 파라미터가 설정 되어 잇지 않기 때문에
    // add() 메서드의 파라미터 타입을 알 수 없다.
    // 그래서 add()를 호출할 때 어떤 타입이 유효한 문법인지 알 수 없어서
    // 컴파일 에러가 발생한다.
    //
    //    list.add(new Object());
    //    list.add(new A());
    ...
    Object obj1 = list.get(0); // 이게 가능한 이유는 이거의 리턴 타입이 뭔지 몰라도 Object에 담을수 잇기 때문이다
